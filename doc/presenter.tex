...
\begin{documentation}
  ...
  \env{frame} or \env{slide} environment to split different pages, but use
  sectioning commands as the natural way to split different points.
  This package will not summarize the contents yet but it is promising to have
  ...
  To use \cls{presenter} is very simple, just use swap the line of \verb"\documentclass{article}"
  ...
  \begin{texnote}
    ...
    or any other document class, use the package \pkg{presenter} right after the
    document class is loaded.
    \begin{verbatim}
      \documentclass{book}
  ...
  \begin{function}[added=2023-03-17]{\LoadPresenterBackground}
    \begin{syntax}
    ...
    \verb"headline". This command could only be used in the preamble.
  \end{function}

  \begin{function}[added=2023-03-17]{\LoadPresenterForeground}
    \begin{syntax}
    ...
    You could specify the optional argument \meta{component comma list} to
    modify one or more components. \meta{component} could be one of
    \verb"headline", \verb"sectioning" and \verb"footline". This command could
