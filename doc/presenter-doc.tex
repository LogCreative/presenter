...
\usepackage{listings}
\lstset{basicstyle=\ttfamily,flexiblecolumns}
% Preview test code and PDF, the first two lines of lvt file will be deleted.
\NewDocumentCommand{\previewtestfull}{O{} m}{
  \begin{center}
  \begin{minipage}[c]{0.45\textwidth}
    \lstinputlisting[firstline=3]{#2.lvt}
  \end{minipage}
...
\begin{itemize}
  \item \textbf{Seamless Migration.} The user should feel seamless on migrating from document class like \cls{article},
  \cls{report} to \cls{presenter} without modifying many structures.
  \item \textbf{Stay On Top.} Sectioning information will stay on the top of the slides.
  \item \textbf{Minimal Design.} The minimal design is for catering to the current trend without introducing too
  many complexities.
\end{itemize}
...
Using \cls{presenter} is very simple, just swap the line of \verb"\documentclass{article}"
into \verb"\documentclass{presenter}" and you are almost done.

\previewtestfull{basics}
